...
export async function ensureWebsiteScreenshotUploaded(submitDir: string): Promise<string> {
	...
	const base = readBaseJson(submitDir);
	const existing =
		typeof base.websiteScreenshot === 'string' ? base.websiteScreenshot.trim() : '';

	if (isRemoteAgentAssetUrl(existing)) {
		console.log(
			`[bat-cli:Screenshot] using existing remote URL, skip upload url=${existing}`,
		);
		return existing;
	}
...
export async function ensureSubmitAssetsUploaded(submitDir: string): Promise<void> {
	...
	console.log(
		`[bat-cli:Assets] submitDir=${submitDir} ready in ${(performance.now() - started).toFixed(0)}ms`,
	);
}
