...
function validateI18nEntry(
  lang: string,
  entry: AgentI18nEntry,
  enEntry: AgentI18nEntry,
): string | null {
...
function validateRelatedArticles(articles: unknown): string | null {
  ...
  for (const item of articles) {
    if (!item || typeof item !== "object")
      return "relatedArticles items must be objects";
    const url = (item as AgentRelatedArticle).url;
    if (typeof url !== "string" || !url.trim())
      return "relatedArticles.url is required";
    try {
      new URL(url);
    } catch {
...
export function agentBundleToEnPost(
  bundle: AgentSubmitBundle,
...
function validateBundleWebsite(
  website: unknown,
): AgentSubmitValidationResult | null {
...
export function validateAgentSubmitPhase1(
  ...
  if (!hasRemoteScreenshot && !hasLocalScreenshot) {
    return {
      ...
      languageErrors:
        Object.keys(languageErrors).length > 0 ? languageErrors : undefined,
    };
  }
  ...
  if (!hasRemoteLogo && !hasLocalLogo) {
    return {
      ...
      items: [],
      errors: {
        logo: "logo URL in base.json or local logo.webp is required (run fetch-logo first)",
      },
      languageErrors:
        Object.keys(languageErrors).length > 0 ? languageErrors : undefined,
    };
...
export function validateAgentSubmitBundle(
  bundle: unknown,
): AgentSubmitValidationResult {
  const languageErrors: Record<string, string> = {};
