...
export function buildSubmitChecklist(
  ...
  const developerName =
    typeof post.developerName === "string" ? post.developerName : "";
  const social = (post.social ?? {}) as { email?: string };
  const coreFeatures = Array.isArray(post.coreFeatures)
    ? post.coreFeatures
    : [];
  ...
  const developerCountry =
    typeof post.developerCountry === "string"
      ? post.developerCountry.trim()
      : "";
  const developerProvince =
    typeof post.developerProvince === "string"
      ? post.developerProvince.trim()
      : "";
  const developerType =
    typeof post.developerType === "string" ? post.developerType.trim() : "";
  ...
  add({
    ...
    ok: optionalDev
      ? isBlank(developerName) || developerName.length >= 2
  ...
  add({
    ...
    ok: Boolean(social.email && /^\S+@\S+\.\S+$/.test(social.email)),
  ...
  const featureTitles = coreFeatures
    .map((f) =>
      typeof f === "object" && f && "title" in f
        ? String(f.title).trim().toLowerCase()
        : "",
    )
    .filter((title) => title);
  ...
  const duplicateFeature = featureTitles.find(
    (title, index) => featureTitles.indexOf(title) !== index,
  );
  const duplicateUseCase = useCaseTitles.find(
    (title, index) => useCaseTitles.indexOf(title) !== index,
  );
  const crossDuplicate = featureTitles.find((title) =>
    useCaseTitles.includes(title),
  );
  ...
  if (pricing.length === 0) {
    ...
    for (let i = 0; i < pricing.length; i++) {
      ...
      if (
        !p.features.every((f) => typeof f === "string" && f.trim().length > 0)
      ) {
...
export function buildSubmitChecklistFromPost(
  post: Record<string, unknown>,
  options: BuildSubmitChecklistOptions = {},
) {
