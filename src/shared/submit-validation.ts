import { isValidDeveloperType } from './developer-type';
import { isUpdateSubmitType, isValidProductSlug, normalizeProductSlug } from './product-slug';
...
export function buildSubmitChecklist(
	...
	const slug = normalizeProductSlug(typeof post.slug === 'string' ? post.slug : '');
	const submitType = typeof post.orderType === 'string' ? post.orderType : typeof post.type === 'string' ? post.type : '';
	const isUpdate = isUpdateSubmitType(submitType) || Boolean(post.productId);
	...
	if (!isUpdate) {
		add({
			...
			sectionId: 'basic-info',
			label: 'URL slug',
			ok: isValidProductSlug(slug),
			message: 'URL slug must be 2-60 characters and use only lowercase letters, numbers, and hyphens',
			errorKey: 'slug',
		});
	}
